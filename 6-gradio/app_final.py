...
def criar_relatorio(temperatura, escala, condicao, texto):
    ...
    relatorio = (
        ...
        f"Temperatura: {temperatura_convertida:.2f}{"F" if escala=="Celsius" else "C"}<br>"
        f"Condição: {condicao}<br>"
        f"Descição: {texto}<br>"
        f"Número de Palavras: {num_palavras}<br>"
        f"Número de Caracteres: {num_caracteres}<br>"
        f"Frequência de Palavras:<br> {frequencia}<br>"
    )
